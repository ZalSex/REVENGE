...
class _DownloaderScreenState extends State<DownloaderScreen> {
  ...
  Future<void> _fetch() async {
    ...
    if (url.isEmpty) { _snack('Masukkan Link Terlebih Dahulu', isError: true); return; }
    ...
    try {
      if (_platform == 0) {
        ...
        final res = await http.get(Uri.parse('https://api.fikmydomainsz.xyz/download/tiktok?url=$encoded'))
        ...
        final encoded = Uri.encodeComponent(url);
        final res = await http.get(Uri.parse('https://api.fikmydomainsz.xyz/download/instagram?url=$encoded'))
            .timeout(const Duration(seconds: 20));
        final json = jsonDecode(res.body);
        if (json['status'] == true) {
          ...
          if (results.isNotEmpty) {
            setState(() {
              ...
              _singleDownload = results[0]['url_download'];
            });
          } else {
        ...
        final encoded = Uri.encodeComponent(url);
        final res = await http.get(Uri.parse('https://api.fikmydomainsz.xyz/download/pinterest?url=$encoded'))
            .timeout(const Duration(seconds: 20));
        final json = jsonDecode(res.body);
        if (json['status'] == true) {
          final r = json['result'];
  ...
  Future<void> _downloadFile(String url, String label) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        _snack('✅ $label Tersimpan: $fileName');
      } else {
