...
class _BugTabState extends State<BugTab> with WidgetsBindingObserver, TickerProviderStateMixin {
  ...
  static const List<Map<String, dynamic>> _methods = [
    {'id': 'forceclose',  'title': 'Force Close',  'icon': AppSvgIcons.zap,   'color': Color(0xFFEF4444)},
    ...
    {'id': 'forceclose2', 'title': 'Force Delete', 'icon': AppSvgIcons.skull, 'color': Color(0xFF10B981)},
    {'id': 'bulldozer',   'title': 'Sedot Kouta',    'icon': AppSvgIcons.drain, 'color': Color(0xFF06B6D4)},
  ];

  final _groupTargetCtrl = TextEditingController();
  final _groupCountCtrl  = TextEditingController(text: '1');
        ...
        if (json['success'] == true) {
          ...
          if (done) {
            ...
            if (error != null && error.isNotEmpty) {
              ...
              _showSnack('Bug Selesai Dikirim ($progress/$total Berhasil)');
              _showSuccessVideo();
            }
          }
        }
                ...
                Flexible(
                  child: ListView.builder(
                    ...
                    itemBuilder: (_, i) {
                      ...
                      final isSelected = _selectedSenderId == s['id'];
                      return GestureDetector(
                        onTap: isOnline ? () {
                          setState(() {
                            _selectedSenderId = s['id'] as String;
                            _selectedSenderPhone = s['phone'] ?? s['id'];
                          });
                            ...
                            children: [
                              ...
                              if (isSelected)
                                Container(
                                  ...
                                  child: Center(
                                    child: SvgPicture.string(AppSvgIcons.zap, width: 12, height: 12,
  ...
  Future<void> _execute() async {
    if (_selectedSenderId == null) { _showSnack('Pilih Sender Terlebih Dahulu', isError: true); return; }
    if (_selectedMethod == null)   { _showSnack('Pilih Metode Bug', isError: true); return; }
    if (_targetCtrl.text.isEmpty)  { _showSnack('Masukkan Nomer Target', isError: true); return; }
    final count = int.tryParse(_countCtrl.text.trim()) ?? 1;
    if (count < 1 || count > 999) { _showSnack('Jumlah Pesan Harus 1–999', isError: true); return; }
    ...
    try {
      ...
      final res = await ApiService.executeBug(
        senderId: _selectedSenderId!, target: _targetCtrl.text.trim(),
      ...
      if (res['success'] == true) {
        ...
        if (jobId != null) {
          ...
          _showSnack('Bug Dikirim, Memantau Status...');
          _startPolling(jobId, isGroup: false);
        } else {
          setState(() => _executing = false);
          _showSnack('Bug Terkirim');
        }
      } else if (res['banned'] == true) {
  ...
  Future<void> _executeGroup() async {
    if (_selectedSenderId == null)    { _showSnack('Pilih Sender Terlebih Dahulu', isError: true); return; }
    if (_selectedGroupMethod == null) { _showSnack('Pilih Metode Bug Grup', isError: true); return; }
    if (_groupTargetCtrl.text.isEmpty){ _showSnack('Masukkan ID Grup Target', isError: true); return; }
    final count = int.tryParse(_groupCountCtrl.text.trim()) ?? 1;
    if (count < 1 || count > 999)    { _showSnack('Jumlah Harus 1–999', isError: true); return; }

    setState(() {
      _executingGroup = true;
      _groupJobStatus = 'running';
      _groupJobProgress = 0;
      _groupJobTotal = count;
      _groupJobError = '';
    });

    try {
      final method = _groupMethods[_selectedGroupMethod!]['id'] as String;
      final res = await ApiService.executeBugGroup(
        senderId: _selectedSenderId!, target: _groupTargetCtrl.text.trim(),
      ...
      if (res['success'] == true) {
        final jobId = res['jobId'] as String?;
        if (jobId != null) {
          setState(() => _currentGroupJobId = jobId);
          _showSnack('Bug Grup Dikirim, Memantau Status...');
          _startPolling(jobId, isGroup: true);
        } else {
          setState(() => _executingGroup = false);
          _showSnack('Bug Grup Terkirim');
        }
      } else if (res['banned'] == true) {
  ...
  void _showSnack(String msg, {bool isError = false}) {
    ...
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(
      ...
      backgroundColor: isError ? Colors.red.shade800 : AppTheme.primaryBlue,
                  ...
                  children: [
                    ...
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          Text(
                            _selectedSenderId != null ? '+${_selectedSenderPhone ?? _selectedSenderId}' : 'Pilih Nomer Sender...',
                            style: TextStyle(
                          ...
                          if (_selectedSenderId != null) ...[
                            ...
                            Row(children: [
                              ...
                              SizedBox(width: 5),
                              Text(tr('sender_terhubung'), style: TextStyle(fontFamily: 'ShareTechMono',
          ...
          Row(
            ...
            children: [
              ...
              Row(children: [
                GestureDetector(
                  onTap: () {
                    final v = int.tryParse(countCtrl.text) ?? 1;
                    if (v > 1) setState(() => countCtrl.text = (v - 1).toString());
                  },
                  child: Container(
                    width: 28, height: 28,
                    decoration: BoxDecoration(
