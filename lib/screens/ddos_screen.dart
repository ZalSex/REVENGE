    ...
    return Scaffold(
      ...
      body: TabBarView(
        ...
        children: [
          ...
          _canMulti
              ? _MultiLayerTab(purple: _purple, purpleDark: _purpleDark)
              : _LockedTab(role: _role),
        ],
      ),
    );
  }
...
class _DdosPanelTabState extends State<_DdosPanelTab> with SingleTickerProviderStateMixin {
  ...
  Future<void> _startAttack() async {
    ...
    if (target.isEmpty) { _snack('Masukkan URL / IP Panel Target', err: true); return; }
    if (!target.startsWith('http://') && !target.startsWith('https://')) { _snack('Link harus diawali http:// atau https://', err: true); return; }
    setState(() => _attacking = true);
    try {
      final res = await ApiService.ddosStart(target: target, time: _timeSeconds);
      if (res['success'] == true) { _attackId = res['attackId']; _snack('Attack Dimulai! Target: $target'); }
  ...
  Future<void> _stopAttack() async {
    try {
      ...
      if (res['success'] == true) { setState(() { _attacking = false; _attackId = null; }); _snack('Attack Dihentikan'); }
  ...
  void _snack(String msg, {bool err = false}) => ScaffoldMessenger.of(context).showSnackBar(SnackBar(
    ...
    backgroundColor: err ? Colors.red.shade900 : c,
...
class _MultiLayerTabState extends State<_MultiLayerTab> with SingleTickerProviderStateMixin {
  ...
  Future<void> _startAttack() async {
    final target = _targetCtrl.text.trim();
    if (target.isEmpty) { _snack('Masukkan URL / IP Target', err: true); return; }
    if (!target.startsWith('http://') && !target.startsWith('https://')) { _snack('Link harus diawali http:// atau https://', err: true); return; }
    if (_selectedMethod == null) { _snack('Pilih Metode Terlebih Dahulu', err: true); return; }
    setState(() => _attacking = true);
    try {
      ...
      if (res['success'] == true) {
        _attackId = res['attackId']; _snack('Multi Layer Dimulai! Target: $target');
  ...
  Future<void> _stopAttack() async {
    ...
    try {
      ...
      _snack(res['success'] == true ? 'Attack Dihentikan, Cooldown Dimulai' : (res['message'] ?? 'Gagal'), err: res['success'] != true);
      if (res['success'] == true) {
