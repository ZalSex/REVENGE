...
class _SpamNglScreenState extends State<SpamNglScreen> {
  ...
  Future<void> _start() async {
    ...
    if (url.isEmpty)   { _snack('Masukkan URL NGL', isError: true); return; }
    if (pesan.isEmpty) { _snack('Masukkan Pesan', isError: true); return; }
    if (!url.contains('ngl.link')) { _snack('URL Harus Dari ngl.link', isError: true); return; }

    final count = int.tryParse(_countCtrl.text) ?? 10;
    if (count < 1 || count > 999) { _snack('Jumlah 1–999', isError: true); return; }
  ...
  void _snack(String msg, {bool isError = false}) {
    ...
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(
      ...
      backgroundColor: isError ? Colors.red.shade800 : _pink,
